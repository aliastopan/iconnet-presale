...
public class WorksheetService
{
    ...
    public byte[] GenerateFullXlsxBytes(IQueryable<WorkPaper>? presaleData, string worksheetName = "Presale Data")
    {
        using var workbook = new XLWorkbook();
        using var memoryStream = new MemoryStream();
        ...
        SetFullWorksheet(worksheet);
        PopulateFullWorksheet(exportModels, worksheet);
    ...
    public byte[] GenerateAgingApprovalXlsxBytes(IQueryable<WorkPaper>? presaleData)
    {
        using var workbook = new XLWorkbook();
        using var memoryStream = new MemoryStream();
        ...
        SetAgingApprovalWorksheet(worksheet);
        PopulateAgingApprovalWorksheet(agingModels, worksheet);
    ...
    private static void SetFullWorksheet(IXLWorksheet worksheet)
    {
        // approval opportunity
        worksheet.Cell(1, 1).Value = "ID PERMOHONAN";
        worksheet.Cell(1, 2).Value = "TGL PERMOHONAN";
        ...
        worksheet.Cell(1, 37).Value = "DIRECT APPROVAL";
        worksheet.Cell(1, 38).Value = "ROOT CAUSE";
        worksheet.Cell(1, 39).Value = "KETERANGAN VERIFIKASI/APPROVAL";
        worksheet.Cell(1, 40).Value = "JARAK SHARE LOC";
        worksheet.Cell(1, 41).Value = "JARAK iCRM+";
        worksheet.Cell(1, 42).Value = "SPLITTER GANTI";
        worksheet.Cell(1, 43).Value = "VA TERBIT";

        worksheet.Cell(1, 44).Value = "PIC IMPORT";
        worksheet.Cell(1, 45).Value = "PIC VERIFIKASI";
        worksheet.Cell(1, 46).Value = "PIC CHAT/CALL PICK-UP";
        worksheet.Cell(1, 47).Value = "PIC VALIDASI";
        worksheet.Cell(1, 48).Value = "PIC APPROVAL";

        worksheet.Cell(1, 49).Value = "WAKTU/TGL IMPORT";
        worksheet.Cell(1, 50).Value = "WAKTU/TGL VERIFIKASI";
        worksheet.Cell(1, 51).Value = "WAKTU/TGL CHAT/CALL PICK-UP";
        worksheet.Cell(1, 52).Value = "WAKTU/TGL VALIDASI";
        worksheet.Cell(1, 53).Value = "WAKTU/TGL APPROVAL";
        ...
        worksheet.Column("AA").Style.DateFormat.Format = _dateTimeFormat;    // waktu/tgl respons
        worksheet.Column("AQ").Style.DateFormat.Format = _dateOnlyFormat;    // va terbit
        worksheet.Column("AW").Style.DateFormat.Format = _dateTimeFormat;     // waktu/tgl import
        worksheet.Column("AX").Style.DateFormat.Format = _dateTimeFormat;    // waktu/tgl verifikasi
        worksheet.Column("AY").Style.DateFormat.Format = _dateTimeFormat;    // waktu/tgl chat/call pick-up
        worksheet.Column("AZ").Style.DateFormat.Format = _dateTimeFormat;    // waktu/tgl validasi
        worksheet.Column("BA").Style.DateFormat.Format = _dateTimeFormat;    // waktu/tgl approval

    }

    private void PopulateFullWorksheet(List<PresaleDataXlsxModel> exportModels, IXLWorksheet worksheet)
    {
        int batchSize = 100;
        int numberOfBatches = (exportModels.Count + batchSize - 1) / batchSize;
        ...
        Parallel.For(0, numberOfBatches, _parallelOptions, batchIndex =>
        {
            ...
            lock (worksheet)
            {
                for (int i = startIndex; i < endIndex; i++)
                {
                    ...
                    worksheet.Cell(row, 44).Value = exportModel.PicImport;
                    worksheet.Cell(row, 45).Value = exportModel.PicVerifikasi;
                    worksheet.Cell(row, 46).Value = exportModel.PicChatCallMulai;
                    worksheet.Cell(row, 47).Value = exportModel.PicChatCallRespons;
                    worksheet.Cell(row, 48).Value = exportModel.PicApproval;

                    worksheet.Cell(row, 49).Value = exportModel.GetTimestampImport();
                    worksheet.Cell(row, 50).Value = exportModel.GetTimestampVerifikasi();
                    worksheet.Cell(row, 51).Value = exportModel.GetTimestampChatCallMulai();
                    worksheet.Cell(row, 52).Value = exportModel.GetTimestampChatCallRespons();
                    worksheet.Cell(row, 53).Value = exportModel.GetTimestampApproval();
                }
            }
        });
    }
    ...
    private static void SetAgingApprovalWorksheet(IXLWorksheet worksheet)
    {
        worksheet.Cell(1, 1).Value = "ID PERMOHONAN";
        worksheet.Cell(1, 2).Value = "TGL PERMOHONAN";
        worksheet.Cell(1, 3).Value = "PIC APPROVAL";
        worksheet.Cell(1, 4).Value = "TGL/WAKTU APPROVAL";
        worksheet.Cell(1, 5).Value = "AGING APPROVAL";
        worksheet.Cell(1, 6).Value = "SLA";
        ...
        worksheet.Column("B").Style.DateFormat.Format = _dateTimeFormat;     // tgl permohonan
        worksheet.Column("D").Style.DateFormat.Format = _dateTimeFormat;     // waktu/tgl approval
        worksheet.Column("E").Style.NumberFormat.Format = _timeSpanFormat;   // aging approval
    }
    ...
    private void PopulateAgingApprovalWorksheet(List<PresaleAgingXlsxModel> agingModels, IXLWorksheet worksheet)
    {
        int batchSize = 100;
        int numberOfBatches = (agingModels.Count + batchSize - 1) / batchSize;
        ...
        Parallel.For(0, numberOfBatches, _parallelOptions, batchIndex =>
        {
            ...
            lock (worksheet)
            {
                for (int i = startIndex; i < endIndex; i++)
                {
                    ...
                    worksheet.Cell(row, 1).Value = exportModel.IdPermohonan;
                    worksheet.Cell(row, 2).Value = exportModel.TglPermohonan;
                    worksheet.Cell(row, 3).Value = exportModel.PicApproval;
                    worksheet.Cell(row, 4).Value = exportModel.TimestampApproval;
                    worksheet.Cell(row, 5).Value = exportModel.AgingApproval;

                    string slaVerdict = exportModel.AgingApproval < _appSettingsService.SlaApproval
                        ? "WON"
                        : "LOST";
